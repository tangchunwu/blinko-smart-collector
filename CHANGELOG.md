...
### ✨ 新增功能
- **🛡️ AI 多模型降级策略 (Multi-model Fallback)**
  - 当首选 AI 模型调用失败时，自动切换到备用模型
  - 优化服务架构，提高 AI 服务的可用性和稳定性

### 🐛 问题修复
- **修复 AI 总结内容提取**
  - 增加多层降级机制，优先使用智能提取，失败时回退到全文
  - 修复部分页面无法获取内容的问题
- **修复 Markdown 渲染问题**
  - 优化空行处理，解决列表项和段落间距过大的问题
  - 自动清理未配对的 `**` 符号，防止显示乱码
- **修复权限警告**
  - 移除无效的 `audioCapture` 权限，消除 Chrome 扩展加载时的警告
- **修复右键菜单**
  - 解决右键菜单项重复添加的问题
