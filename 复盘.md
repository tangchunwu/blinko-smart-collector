...
**问题描述**：
点击"AI总结"按钮时，生成的摘要内容很短，只包含页面标题相关信息，没有总结整个页面的核心内容。

**根因分析**：
在 `sidepanel.js` 的 `generateAISummary()` 方法中，当 `getPageContent` 消息通信失败或返回空内容时，代码回退使用 `currentTab.title` 作为 AI 输入：
```javascript
pageContent = currentTab?.title || '';  // ❌ 只拿标题
```

**解决方案**：
1. 在 `content-sidepanel.js` 新增 `getFullPageText` action，直接返回 `document.body.innerText`
2. 修改 `sidepanel.js` 的回退逻辑：
   - 优先使用 `ContentExtractor` 提取的智能内容
   - 如果内容太短（<100字），调用 `getFullPageText` 获取完整页面文本
   - 添加清晰的错误提示，引导用户刷新页面

**学到的经验**：
- 涉及内容提取的功能，必须有**多层回退机制**
- 用户操作后没反应时，应该给出明确的错误信息而非静默失败
...
**问题描述**：
AI 生成的摘要使用 Markdown 渲染后，列表项之间、段落之间的空白非常大，阅读体验差。

**根因分析**：
1. `markdown-parser.js` 将**所有换行符**都替换为 `<br>` 标签，包括列表项 `</li>\n<li>` 之间的换行
2. `sidepanel.css` 中没有定义 `<p>` 标签的样式，使用浏览器默认的 `margin: 1em`

**解决方案**：
1. **修改解析器** (`markdown-parser.js`)：
   - 在插入 `<br>` 前，先清理列表内的换行：`</li>\n+<li>` → `</li><li>`
   - 清理列表周围的 `<br>` 标签
2. **优化 CSS** (`sidepanel.css`)：
   - 段落 `p`: `margin: 4px 0`
   - 标题 `h1-h3`: `margin: 8px 0 4px 0`
   - 列表 `ul/ol`: `margin: 4px 0`
   - 列表项 `li`: `margin: 2px 0`

**学到的经验**：
- Markdown 解析器的换行处理需要**区分块级元素和行内元素**
- 自定义 Markdown 渲染时，必须重置浏览器默认样式
...
**问题描述**：
AI 生成的内容中，有些 `**` 不是成对出现的（如 `选型决策树**:`），导致粗体语法无法匹配，`**` 原样显示在页面上。

**根因分析**：
正则表达式 `/\*\*(.+?)\*\*/g` 只能匹配**成对**的 `**...**`，落单的 `**` 不会被处理。

**解决方案**：
在粗体/斜体处理完成后，添加一行清理逻辑：
```javascript
html = html.replace(/\*\*/g, '');  // 移除所有剩余的 **
```

**学到的经验**：
- AI 生成的 Markdown 不一定规范，解析器需要有**容错清理**机制
- 与其让用户看到乱码般的符号，不如直接移除

---

### 🔧 Chrome 扩展警告：audioCapture 权限无效

**问题描述**：
加载扩展时 Chrome 显示警告：`'audioCapture' is only allowed for packaged apps, but this is a extension.`

**根因分析**：
`manifest.json` 中声明了 `"audioCapture"` 权限，但该权限只适用于 **Chrome Apps（打包应用）**，不能用于 **Chrome Extensions（扩展）**。

**解决方案**：
从 `manifest.json` 的 `permissions` 数组中移除 `"audioCapture"`。

**补充知识**：
扩展中实现语音输入有两种方式，都不需要 `audioCapture` 权限：
| 方案 | 原理 |
|------|------|
| **Web Speech API** | 浏览器原生 API，通过用户授权弹窗获取麦克风权限 |
| **云端 API (如 Soniox)** | 通过 `getUserMedia` 获取音频流，发送到云端识别 |
